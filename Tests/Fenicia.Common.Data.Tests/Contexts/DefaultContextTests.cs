...
public class DefaultContextTests : IDisposable
{
    ...
    public async Task SaveChangesAsync_ShouldSetCreated_OnAddedEntity()
    {
        var company = new CompanyModel
            { Name = _faker.Company.CompanyName(), Cnpj = _faker.Company.Cnpj(), IsActive = true };
    ...
    public async Task GetByIdAsync_ShouldExcludeSoftDeleted_ForBaseModel()
    {
        var user = new UserModel
            { Email = _faker.Internet.Email(), Name = _faker.Person.FullName, Password = _faker.Internet.Password() };
    ...
    public async Task SoftDeletedEntity_ShouldBeIncluded_WhenIgnoringQueryFilters()
    {
        ...
        var result = await _db.AuthUsers.IgnoreQueryFilters()
    ...
    public async Task BaseCompanyModel_ShouldBeFilteredByCompanyId_WhenCurrentCompanyIdIsSet()
    {
        ...
        var company1 = new CompanyModel
            { Id = Guid.NewGuid(), Name = _faker.Company.CompanyName(), Cnpj = _faker.Company.Cnpj(), IsActive = true };
        ...
        var config1 = new ConfigurationModel
            { CompanyId = company1.Id, ConfigType = ConfigType.Language, Value = "Value1", UserId = Guid.NewGuid() };
        var config2 = new ConfigurationModel
            { CompanyId = company2.Id, ConfigType = ConfigType.Language, Value = "Value2", UserId = Guid.NewGuid() };
