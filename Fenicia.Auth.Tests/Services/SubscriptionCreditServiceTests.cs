...
public class SubscriptionCreditServiceTests
{
    ...
    public void Setup()
    {
        ...
        this.sut = new SubscriptionCreditService(this.subscriptionCreditRepositoryMock.Object,
            this.subscriptionServiceMock.Object);
    }
    ...
    public async Task GetActiveModulesTypesAsyncWhenValidSubscriptionsExistReturnsModuleTypes()
    {
        ...
        this.subscriptionServiceMock.Setup(x => x.GetValidSubscriptionsAsync(companyId, this.cancellationToken))
            .ReturnsAsync(validSubscriptions);

        this.subscriptionCreditRepositoryMock
            .Setup(x => x.GetValidModulesTypesAsync(validSubscriptions, this.cancellationToken))
        ...
        this.subscriptionServiceMock.Verify(x => x.GetValidSubscriptionsAsync(companyId, this.cancellationToken),
            Times.Once);
        this.subscriptionCreditRepositoryMock.Verify(
            x => x.GetValidModulesTypesAsync(validSubscriptions, this.cancellationToken), Times.Once);
    }
    ...
    public async Task GetActiveModulesTypesAsyncWhenNoValidSubscriptionsReturnsFailureResponse()
    {
        ...
        this.subscriptionServiceMock.Verify(x => x.GetValidSubscriptionsAsync(companyId, this.cancellationToken),
            Times.Once);
        this.subscriptionCreditRepositoryMock.Verify(
            x => x.GetValidModulesTypesAsync(It.IsAny<List<Guid>>(), this.cancellationToken), Times.Never);
    }
