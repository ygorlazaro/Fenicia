...
const ProjectStatusList = () => {
    ...
    return (
        <CContainer className="py-4">
                                ...
                                <CTableBody>
                                    {statuses.map((status) => (
                                        <CTableRow key={status.id}>
                                            ...
                                            <CTableDataCell>
                                                <CBadge color={getStatusBadgeColor(status.color)}>{status.color}</CBadge>
                                            </CTableDataCell>
                                            <CTableDataCell>{status.order}</CTableDataCell>
                                            <CTableDataCell>
                                                <CFormSwitch checked={status.isFinal} disabled />
                                            </CTableDataCell>
                                            <CTableDataCell className="text-end">
                                                <CButton color="info" size="sm" className="me-2" onClick={() => handleOpenEdit(status)}>
                                                    <CIcon icon={cilPencil} />
                                                </CButton>
                                                <CButton color="danger" size="sm" onClick={() => handleOpenDelete(status)}>
                                                    <CIcon icon={cilTrash} />
                                                </CButton>
                                            </CTableDataCell>
                                        </CTableRow>
            ...
            {/* Add/Edit Modal */}
            <ProjectStatusModal visible={modalVisible} onClose={() => setModalVisible(false)} onSave={handleSave} projectStatus={selectedStatus} loading={saving} />
