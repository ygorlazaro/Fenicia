import { CAlert, CButton, CCard, CCol, CForm, CFormCheck, CFormInput, CFormLabel, CModal, CModalBody, CModalFooter, CModalHeader, CModalTitle, CRow, CSpinner } from "@coreui/react";
...
const UserModal = ({ visible, onClose, onSave, mode, user }) => {
    ...
    const handleCompanyRoleToggle = (companyId, roleId) => {
        const exists = formData.companiesRoles.find((cr) => cr.companyId === companyId && cr.roleId === roleId);

        if (exists) {
            setFormData((prev) => ({
                ...prev,
                companiesRoles: prev.companiesRoles.filter((cr) => !(cr.companyId === companyId && cr.roleId === roleId))
            }));
        } else {
            setFormData((prev) => ({
                ...prev,
    ...
    return (
        <CModal visible={visible} onClose={onClose} size="lg">
            <CModalHeader>
                <CModalTitle>{mode === "create" ? t("users.createTitle") || "Create New User" : t("users.editTitle") || "Edit User"}</CModalTitle>
            </CModalHeader>
            <CModalBody>
                {error && (
                ...
                <CForm onSubmit={handleSubmit}>
                    <CRow className="mb-3">
                        <CCol md={6}>
                            ...
                            <CFormInput id="name" name="name" value={formData.name} onChange={handleInputChange} required />
                        </CCol>
                        <CCol md={6}>
                            <CFormLabel htmlFor="email">{t("users.email") || "Email"}</CFormLabel>
                            <CFormInput id="email" name="email" type="email" value={formData.email} onChange={handleInputChange} required />
                        </CCol>
                    </CRow>
                    ...
                    {mode === "create" && (
                        <CRow className="mb-3">
                            <CCol md={12}>
                                <CFormLabel htmlFor="password">{t("users.password") || "Password"}</CFormLabel>
                                <CFormInput id="password" name="password" type="password" value={formData.password} onChange={handleInputChange} required minLength={6} />
                            </CCol>
                        </CRow>
                    )}
                                        ...
                                        <CCard className="mb-2 p-2" key={company.id}>
                                                ...
                                                onChange={() => {
                                                    ...
                                                    if (formData.companiesRoles.some((cr) => cr.companyId === company.id)) {
                                                        ...
                                                        if (firstRole) {
                                                            setFormData((prev) => ({
                                                                ...prev,
                                                                companiesRoles: [...prev.companiesRoles, { companyId: company.id, roleId: firstRole.id }]
                                                            }));
                                                        }
                                                    }
                                                }}
                                            ...
                                            {formData.companiesRoles.some((cr) => cr.companyId === company.id) && (
                                                <div className="ms-4 mt-2">
                                                    <CFormLabel className="small text-muted">{t("users.selectRole") || "Select Role:"}</CFormLabel>
                                                    <select
                                                        className="form-select form-select-sm"
                                                        value={formData.companiesRoles.find((cr) => cr.companyId === company.id)?.roleId || ""}
                                                        onChange={(e) => {
                                                            const newRoleId = e.target.value;
                                                            setFormData((prev) => ({
                                                                ...prev,
                                                                companiesRoles: prev.companiesRoles.map((cr) => (cr.companyId === company.id ? { ...cr, roleId: newRoleId } : cr))
                                                            }));
                                                        }}
                                                    >
                                                        {roles.map((role) => (
