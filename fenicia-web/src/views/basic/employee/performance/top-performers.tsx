...
import { TopPerformer } from "../../../../types/basic/employee/top-performer";
                                ...
                                <CTableBody>
                                    {topPerformers.map((performer, index) => (
                                        <CTableRow key={performer.employeeId}>
                                            <CTableDataCell>{index <= 2 ? <CIcon icon={cilTruck} className={`text-${index === 0 ? "warning" : index === 1 ? "secondary" : "danger"}`} size="lg" /> : index + 1}</CTableDataCell>
                                            <CTableDataCell>
                                            ...
                                            <CTableDataCell className="text-center">
                                                <CBadge color={getPerformanceLevelColor(performer.performanceLevel)}>{t(`employees.${performer.performanceLevel.toLowerCase().replace(" ", "")}`)}</CBadge>
                                            </CTableDataCell>
                                        </CTableRow>
                                    ))}
                                </CTableBody>
