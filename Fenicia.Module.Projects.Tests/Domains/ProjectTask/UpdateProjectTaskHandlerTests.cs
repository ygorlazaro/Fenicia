...
public class UpdateProjectTaskHandlerTests : IDisposable
{
    ...
    public async Task Handle_WithMultipleUpdates_UpdatesCorrectProjectTask()
    {
        ...
        Assert.Equal(task1Id,
            result.Id);
        Assert.Equal("Updated Task 1 Title",
            result.Title);

        var updatedTask1 = await this.db.ProjectTasks.FindAsync([
                task1Id
            ],
            CancellationToken.None);
        var task2InDb = await this.db.ProjectTasks.FindAsync([
                task2Id
            ],
            CancellationToken.None);
