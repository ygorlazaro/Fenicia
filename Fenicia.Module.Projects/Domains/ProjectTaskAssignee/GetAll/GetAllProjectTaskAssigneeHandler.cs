...
public class GetAllProjectTaskAssigneeHandler(DefaultContext context)
{
    ...
    {
        return await context.ProjectTaskAssignees.Select(a => new GetAllProjectTaskAssigneeResponse(a.Id, a.TaskId, a.UserId, a.Role.ToString(), a.AssignedAt, a.CompanyId)).Skip((query.Page - 1) * query.PerPage).Take(query.PerPage).ToListAsync(ct);
    }
}