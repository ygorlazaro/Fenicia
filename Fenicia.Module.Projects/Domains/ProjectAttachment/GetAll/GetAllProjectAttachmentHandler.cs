...
public class GetAllProjectAttachmentHandler(DefaultContext context)
{
    ...
    {
        return await context.ProjectAttachments.Select(p => new GetAllProjectAttachmentResponse(p.Id, p.TaskId, p.FileName, p.FileUrl, p.FileSize, p.UploadedBy, p.CompanyId)).Skip((query.Page - 1) * query.PerPage).Take(query.PerPage).ToListAsync(ct);
    }
}