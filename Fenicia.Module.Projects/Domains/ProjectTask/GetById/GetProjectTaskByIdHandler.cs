...
public class GetProjectTaskByIdHandler(DefaultContext context)
{
    ...
    {
        var projectTask = await context.ProjectTasks.Include(pt => pt.Attachments).Include(pt => pt.Comments).Include(pt => pt.Subtasks).Include(pt => pt.Assignees).ThenInclude(a => a.User).FirstOrDefaultAsync(pt => pt.Id == query.Id, ct);
