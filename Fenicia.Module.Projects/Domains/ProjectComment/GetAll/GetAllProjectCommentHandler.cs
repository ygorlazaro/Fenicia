...
public class GetAllProjectCommentHandler(DefaultContext context)
{
    ...
    {
        return await context.ProjectComments.Select(pc => new GetAllProjectCommentResponse(pc.Id, pc.TaskId, pc.UserId, pc.Content, pc.CompanyId)).Skip((query.Page - 1) * query.PerPage).Take(query.PerPage).ToListAsync(ct);
    }
}