...
public class ShareController(ShareService shareService) : ControllerBase
{
    ...
    public async Task<ActionResult<AddShareResponse>> PostAsync([FromBody] ShareCommand command, WideEventContext wide, CancellationToken ct)
    {
        wide.UserId = ClaimReader.UserId(User).ToString();

        var share = await shareService.ShareAsync(command, ClaimReader.UserId(User), ClaimReader.UserId(User), ct);
    ...
    public async Task<ActionResult<List<GetSharesResponse>>> GetSharesByFeedAsync([FromRoute] Guid feedId, WideEventContext wide, [FromQuery] int page = 1, [FromQuery] int perPage = 10, CancellationToken ct)
    {
        ...
        var shares = await shareService.GetSharesByFeedAsync(new GetSharesByFeedQuery(page, perPage), feedId, ct);
