            ...
            <CCardBody>
                {items.length === 0 ? (
                    ...
                    <CTable align="middle" className="mb-0 border" hover responsive>
                        <CTableHead>
                            <CTableRow>
                                <CTableHeaderCell className="bg-body-tertiary">{t("inventory.productName")}</CTableHeaderCell>
                                <CTableHeaderCell className="bg-body-tertiary text-center">{t("inventory.category")}</CTableHeaderCell>
                                <CTableHeaderCell className="bg-body-tertiary text-center">{t("inventory.quantity")}</CTableHeaderCell>
                                <CTableHeaderCell className="bg-body-tertiary text-end">{t("inventory.costPrice")}</CTableHeaderCell>
                                <CTableHeaderCell className="bg-body-tertiary text-end">{t("inventory.salesPrice")}</CTableHeaderCell>
                                <CTableHeaderCell className="bg-body-tertiary text-center">{t("inventory.status")}</CTableHeaderCell>
                            </CTableRow>
                        </CTableHead>
                        <CTableBody>
                            {items.map((item) => {
                                ...
                                return (
                                    <CTableRow key={item.id}>
                                        ...
                                        <CTableDataCell className="text-end">{formatCurrency(item.costPrice ?? 0)}</CTableDataCell>
                                        <CTableDataCell className="text-end">{formatCurrency(item.salesPrice)}</CTableDataCell>
                                        <CTableDataCell className="text-center">
                                            <CProgress thin color={stockLevel.color} value={Math.min(item.quantity * 10, 100)} style={{ width: "60px", display: "inline-block" }} />
