import { CAlert, CButton, CCard, CCardBody, CCardHeader, CCol, CContainer, CForm, CFormCheck, CRow, CSpinner } from "@coreui/react";
...
const Subscription = () => {
    ...
    const newSelectedModules = useMemo(() => selectedModules.filter((id) => !subscribedModuleIds.includes(id)), [selectedModules, subscribedModuleIds]);
    ...
    const loadModules = async () => {
        try {
            ...
            const [modulesResponse, subscribedIds, profile] = await Promise.all([moduleClient.getModules(1, 50), moduleClient.getSubscribedModuleIds(), profileClient.getProfile()]);
            ...
            const nonBasicIds = profile?.subscriptions?.flatMap((subscription) => subscription.modules.filter((m) => m.type !== "Basic").map((m) => m.id)) || [];
                                ...
                                <>
                                    ...
                                    <CForm onSubmit={handleSubmit}>
                                            ...
                                            {modules.map((module) => {
                                                ...
                                                return (
                                                    <CCol md={6} lg={4} key={module.id}>
                                                        <CCard
                                                            className={`h-100 shadow-sm ${isSelected && !isSubscribed ? "border-primary bg-primary-subtle" : isSubscribed ? "border-success bg-success-subtle" : "border-light"}`}
                                                            style={{
                                                                ...
                                                                <CFormCheck
                                                                    ...
                                                                    label={
                                                                        <>
                                                                            ...
                                                                            {isSubscribed && <span className="ms-2 badge bg-success">Já assinado</span>}
                                                                        </>
                                                                    }
                                                                    checked={isSelected}
                                                                    ...
                                                                    className="mb-2"
                                                                />
                                                                <div className="fw-bold text-success h5 mb-1">{formatCurrency(module.price || 0)}/mês</div>
                                                            </CCardBody>
                                                        </CCard>
                                                    </CCol>
                                                );
                                        ...
                                        <div className="mt-4 d-flex gap-2">
                                            <CButton color="primary" type="submit" disabled={ordering || selectedCountNew === 0}>
                                                {ordering ? "Processando..." : "Criar Assinatura"}
                                            </CButton>
                                            <CButton color="secondary" type="button" onClick={() => navigate("/dashboard")}>
                                                Cancelar
                                            </CButton>
                                        </div>
                                    </CForm>
