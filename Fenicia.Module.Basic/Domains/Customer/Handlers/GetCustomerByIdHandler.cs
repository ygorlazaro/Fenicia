...
public class GetCustomerByIdHandler(DefaultContext db)
{
    ...
    {
        var customer = await db.BasicCustomers.Include(c => c.Person).FirstOrDefaultAsync(c => c.Id == query.Id, ct);

        return customer switch
        {
            null => null,
            _ => new GetCustomerByIdResponse(customer.Id, customer.PersonId, customer.Person.Name, customer.Person.Email, customer.Person.PhoneNumber, customer.Person.Document, customer.Person.Street, customer.Person.Number, customer.Person.Complement, customer.Person.Neighborhood, customer.Person.ZipCode, customer.Person.StateId, customer.Person.City)
        };
    }
}