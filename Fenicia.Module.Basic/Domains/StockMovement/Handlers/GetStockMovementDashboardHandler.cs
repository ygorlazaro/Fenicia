...
public class GetStockMovementDashboardHandler(DefaultContext db)
{
    public async Task<StockMovementDashboardResponse> Handle(GetStockMovementDashboardQuery query, CancellationToken ct)
    {
        ...
        var movements = db.BasicStockMovements
    ...
    private async Task<List<StockTurnoverResponse>> GetStockTurnoverAsync(
        ...
        IQueryable<StockMovementModel> movements,
        CancellationToken ct)
    {
        var productOutMovements =
            from m in movements
            where m.Type == StockMovementType.Out
            group m by m.ProductId
            into g
            select new
            {
                ProductId = g.Key,
        ...
        var request =
            ...
            where p.Quantity > 0
            join m in productOutMovements
                on p.Id equals m.ProductId into gj
        ...
        return data
            .Select(x => new StockTurnoverResponse(
                x.Id,
                x.Name,
                x.CategoryName,
                x.Quantity,
    ...
    private async Task<List<TopMovedProductResponse>> GetTopMovedProductAsync(
        GetStockMovementDashboardQuery query,
        IQueryable<StockMovementModel> movements,
        CancellationToken ct)
    {
        var request =
            ...
            select new TopMovedProductResponse(
                g.Key.ProductId,
                g.Key.ProductName,
                g.Key.CategoryName,
                g.Sum(x => x.Quantity),
    ...
    private async Task<List<MonthlyInOutResponse>> GetMonthlyInOutAsync(
        IQueryable<StockMovementModel> movements,
        CancellationToken ct)
    {
        var query =
        ...
        return data
            .Select(x => new MonthlyInOutResponse(
                $"{x.Month:D2}/{x.Year}",
                x.TotalIn,
                x.TotalOut,
                x.TotalInValue,
    ...
    private async Task<List<StockMovementHistoryResponse>> GetStockMovementHistoryAsync(
        IQueryable<StockMovementModel> movements,
        CancellationToken ct)
    {
