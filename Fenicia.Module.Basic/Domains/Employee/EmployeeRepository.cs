...
{
    public async Task<List<EmployeeModel>> GetByPositionIdAsync(
        Guid positionId,
        CancellationToken ct,
        int page = 1,
        int perPage = 10)
    {
        return await context.Employees.Where(e => e.PositionId == positionId).Skip((page - 1) * perPage).Take(perPage)
            .ToListAsync(ct);
    }
}