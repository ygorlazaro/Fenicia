...
public class GetInventoryByProductHandler(DefaultContext db)
{
    public async Task<InventoryResponse> Handle(GetInventoryByProductQuery query, CancellationToken ct)
    {
        ...
        var totalCostPrice = await db.BasicProducts.Where(p => p.Id == query.ProductId).SumAsync(
            p => p.CostPrice ?? 0,
            ct);
        var totalSalesPrice = await db.BasicProducts.Where(p => p.Id == query.ProductId).SumAsync(
            p => p.SalesPrice,
            ct);
        var totalQuantity = await db.BasicProducts.Where(p => p.Id == query.ProductId).SumAsync(p => p.Quantity,
            ct);
