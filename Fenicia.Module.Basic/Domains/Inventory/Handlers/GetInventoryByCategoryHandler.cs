...
public class GetInventoryByCategoryHandler(DefaultContext db)
{
    public async Task<InventoryResponse> Handle(GetInventoryByCategoryQuery query, CancellationToken ct)
    {
        ...
        var totalCostPrice = await db.BasicProducts.Where(p => p.CategoryId == query.CategoryId).SumAsync(
            p => p.CostPrice ?? 0,
            ct);
        var totalSalesPrice = await db.BasicProducts.Where(p => p.CategoryId == query.CategoryId).SumAsync(
            p => p.SalesPrice,
            ct);
        var totalQuantity = await db.BasicProducts.Where(p => p.CategoryId == query.CategoryId).SumAsync(
            p => p.Quantity,
            ct);
