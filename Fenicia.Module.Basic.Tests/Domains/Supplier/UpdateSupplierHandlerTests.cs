...
public class UpdateSupplierHandlerTests : IDisposable
{
    ...
    public async Task Handle_WhenSupplierExists_UpdatesSupplierAndReturnsResponse()
    {
        ...
        var supplier = new SupplierModel { Id = supplierId, Cnpj = "12.345.678/0001-90", PersonId = Guid.NewGuid(), Person = new PersonModel { Id = Guid.NewGuid(), Name = "Old Name", Email = "old@email.com" } };
    ...
    public async Task Handle_VerifiesSupplierWasUpdatedInDatabase()
    {
        ...
        // Assert
        var updatedSupplier = await this.db.BasicSuppliers.Include(s => s.Person).FirstOrDefaultAsync(s => s.Id == supplierId);
