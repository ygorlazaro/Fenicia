...
public class BlockControllerTests : IDisposable
{
    ...
    public BlockControllerTests()
    {
        ...
        _controller = new BlockController(_mockService.Object, profileService.Object) { ControllerContext = new ControllerContext { HttpContext = _mockHttpContext.Object } };
        _faker = new Faker();
    ...
    private void SetupServiceMocks()
    {
        _mockService.Setup(s => s.BlockAsync(It.IsAny<BlockCommand>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((BlockCommand cmd, Guid userId, CancellationToken cancellationToken) => new AddBlockResponse(Guid.NewGuid(), userId, cmd.BlockedProfileId, DateTime.UtcNow, null, true));

        _mockService.Setup(s => s.GetBlockedAsync(It.IsAny<GetBlockedQuery>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Pagination<List<GetBlockedResponse>>([], 0, 1, 10));
