...
public class CommentServiceTests : IDisposable
{
    ...
    public CommentServiceTests()
    {
        ...
        var feedRepository = new Mock<FeedRepository>();
        _service = new CommentService(new CommentRepository(_db), feedRepository.Object);
        _faker = new Faker();
    ...
    public async Task GetAllByFeedAsync_WhenCommentsExist_ReturnsPaginationWithComments()
    {
        ...
        // Act
        var result = await _service.GetAllByFeedAsync(new GetAllCommentByFeedQuery(1, 10, comment.FeedId), comment.FeedId, comment.ProfileId, CancellationToken.None);
    ...
    public async Task GetAllByFeedAsync_OrdersByCommentDateAscending()
    {
        ...
        var comment1 = new CommentModel { Id = Guid.NewGuid(), ProfileId = Guid.NewGuid(), FeedId = feedId, Text = _faker.Lorem.Sentence(), CommentDate = DateTime.UtcNow.AddDays(-2), CompanyId = _companyId };
        var comment2 = new CommentModel { Id = Guid.NewGuid(), ProfileId = Guid.NewGuid(), FeedId = feedId, Text = _faker.Lorem.Sentence(), CommentDate = DateTime.UtcNow.AddDays(-1), CompanyId = _companyId };
        var comment3 = new CommentModel { Id = Guid.NewGuid(), ProfileId = Guid.NewGuid(), FeedId = feedId, Text = _faker.Lorem.Sentence(), CommentDate = DateTime.UtcNow, CompanyId = _companyId };
        _db.SocialNetworkComments.AddRange(comment1, comment2, comment3);
        await _db.SaveChangesAsync(CancellationToken.None);

        // Act
        var result = await _service.GetAllByFeedAsync(new GetAllCommentByFeedQuery(1, 10, feedId), feedId, comment1.ProfileId, CancellationToken.None);
    ...
    public async Task AddAsync_WhenCommandIsValid_CreatesComment()
    {
        ...
        var command = new AddCommentCommand(Guid.NewGuid(), profileId, Guid.NewGuid(), null, _faker.Lorem.Sentence());

        // Act
        var result = await _service.AddAsync(command, _companyId, profileId, CancellationToken.None);
    ...
    public async Task UpdateAsync_WhenCommentExists_UpdatesComment()
    {
        ...
        // Act
        var result = await _service.UpdateAsync(command, profileId, CancellationToken.None);
    ...
    public async Task UpdateAsync_WhenUserIsNotOwner_ReturnsNull()
    {
        ...
        // Act
        var result = await _service.UpdateAsync(command, otherProfileId, CancellationToken.None);
    ...
    public async Task DeleteAsync_WhenCommentExists_SoftDeletesComment()
    {
        ...
        // Act
        await _service.DeleteAsync(new DeleteCommentCommand(comment.Id), profileId, CancellationToken.None);
    ...
    public async Task DeleteAsync_WhenUserIsNotOwner_DoesNothing()
    {
        ...
        // Act
        await _service.DeleteAsync(new DeleteCommentCommand(comment.Id), otherProfileId, CancellationToken.None);
    ...
    public async Task GetRepliesAsync_WhenRepliesExist_ReturnsReplies()
    {
        ...
        // Act
        var result = await _service.GetRepliesAsync(new GetRepliesQuery(1, 10, parentCommentId, null, null), reply.ProfileId, CancellationToken.None);
