...
public class FriendshipControllerTests : IDisposable
{
    ...
    public FriendshipControllerTests()
    {
        ...
        var profileService = new Mock<ProfileService>();
        _controller = new FriendshipController(_mockService.Object, profileService.Object) { ControllerContext = new ControllerContext { HttpContext = _mockHttpContext.Object } };
        _faker = new Faker();
    ...
    private void SetupServiceMocks()
    {
        _mockService.Setup(s => s.FollowAsync(It.IsAny<FollowCommand>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((FollowCommand cmd, Guid userId, CancellationToken cancellationToken) => new AddFriendshipResponse(Guid.NewGuid(), userId, cmd.TargetProfileId, DateTime.UtcNow, true));

        _mockService.Setup(s => s.GetFollowersAsync(It.IsAny<GetFollowersQuery>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Pagination<List<GetFollowersResponse>>([], 0, 1, 10));

        _mockService.Setup(s => s.GetFollowingAsync(It.IsAny<GetFollowingQuery>(), It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new Pagination<List<GetFollowingResponse>>([], 0, 1, 10));
